...
function recalculate_user_badges(PDO $pdo, int $userId): void {
    ...
    // 6) Code Defender — NOUVEAU MOIS 7
    $stmt = $pdo->prepare("
        SELECT completed FROM module_progress
        WHERE user_id = ? AND module_code = 'secure_dev' AND completed = 1
    ");
    $stmt->execute([$userId]);
    if ($stmt->fetch()) award_badge($pdo, $userId, 'code_defender');

    // 7) Speed Demon — NOUVEAU MOIS 7
    $stmt = $pdo->prepare("
        SELECT COUNT(*) AS c FROM challenge_sessions
        WHERE user_id = ?
          AND total_questions > 0
          AND (correct_answers / total_questions) >= 0.80
          AND duration_seconds <= 45
          AND finished_at IS NOT NULL
    ");
    $stmt->execute([$userId]);
    if ($stmt->fetch()['c'] > 0) award_badge($pdo, $userId, 'speed_demon');
}